...
def laplacian_pyr_join(pyr1, pyr2):
    ...
    for i in range(len(pyr1)):
        left = pyr1[i]
        right = pyr2[i]
        layer = np.zeros(left.shape, left.dtype) # uint32

        y, x, _ = left.shape # DEBUG
        ...
        if y % 2 == 1: # DEBUG x
            ...
            layer[:half, :, ...] = left[:half, :, ...] # DEBUG, currently horizontal blending
            layer[-half:, :, ...] = right[-half:, :, ...]
            ''' blend center pixel '''
            ...
            layer[half,: , ...] = (left[half, :, ...] + right[half, :, ...]) # DEBUG, currently horizontal blending
        else:
            ''' even width '''
            half = y // 2 # DEBUG x
            ''' assign halves '''
...
def main():
    start = time.time()
    plt.ion()
    #im1 = matplotlib.image.imread('./data/test1.jpg')
    #im2 = matplotlib.image.imread('./data/test2.jpg')
